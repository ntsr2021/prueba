...
// Verificar si hai un banner abajo del que se desea bajar posicion
$sql = "SELECT * FROM banner_principal WHERE posicion=($posicion+1)";
$cons = $conex->query($sql);


$mens = '';
if($cons->fetch_array()) // Si hai un banner abajo 
{
	// Subir el banner que esta abajo
	$sql = "UPDATE banner_principal SET posicion=($posicion) WHERE posicion=($posicion+1)";
	$conex->query($sql);

	// Bajar banner seleccionado
